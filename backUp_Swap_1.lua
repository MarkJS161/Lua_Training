...
function check_magnetic_input (cardInfo, cardLength)
    ...
    elseif string.len(lic_track_1) ~= 0 and string.len(lic_track_2) ~= 0 and string.len(lic_track_3) == 0 then
        ...
        ev = avl.pfal("MSG.Send.USB,0,\"GET 2 TRACKS INSIDE; MISS TRACK#3\"")
        getDriverName(lic_track_1)
        getCardDate(lic_track_2)
    ...
    elseif string.len(lic_track_1) == 0 and string.len(lic_track_2) == 0 and string.len(lic_track_3) ~= 0 then
        ...
        ev = avl.pfal("MSG.Send.USB,0,\"GET ONLY 1 TRACK INSIDE; MISS TRACK# 1 & 2\"")
        getLicType(lic_track_3)

    else
        ev = avl.pfal("MSG.Send.USB,0,\"CANNOT GET ALL TRACKS INSIDE;\"")
    end
end
...
function getDriverName(track_1)
    ...
    -- find MR., MS., MRS. to set length for search first, last name in card
    if mr ~= nil then
...
function getCardDate(track_2)
    ...
    if track_2_len > 1 then
        ...
        if index == 4 then
            data = data..string.format("The country code : %s.", country)
            data = data..string.format("The ID No. : %s, %d.", idNo, string.len(idNo))
            data = data..string.format("The Date of Expiry : M:%s-Y:%s.", string.sub(expiry, 3), string.sub(expiry, 1, 2))
            data = data..string.format("The Date of Birth : D:%s-M:%s-Y:%s.", string.sub(dateBirth,7), string.sub(dateBirth, 5, 6), string.sub(dateBirth, 1, 4))
    ...
    if length > 0 then
        ...
        if string.len(lic_index[1]) == 2 then
            ...
            if lic_index[1] == "21" or lic_index[1] == "22" or lic_index[1] == "23" then
                ev = avl.pfal(string.format("MSG.Send.USB,0,\"OH YEAH!!!, BEST MATCH License type : %s\"", lic_index[1]))
            ...
            ev = avl.pfal(string.format("MSG.Send.USB,0,\"NO BEST MATCH, License type : %s\"", lic_index[1]))
        end
        ...
        if trackIndex >= 4 then
            ...
            ev = avl.pfal(string.format("MSG.Send.USB,0,\"License Type : %s\"", lic_index[1]))
            ev = avl.pfal(string.format("MSG.Send.USB,0,\"Gender (Male = 1 , Female = 2, Unknown = 3) : %s\"", lic_index[2]))
            ev = avl.pfal(string.format("MSG.Send.USB,0,\"License No. : %s\"", lic_index[3]))
            ev = avl.pfal(string.format("MSG.Send.USB,0,\"Location No. : %s\"", lic_index[4]))
        else 
            -- trackIndex not complete (Not Equals == 4)
            ev = avl.pfal("MSG.Send.USB,0,\"License Information Not complete (some information is lost)\n\"")
            data = data..("License Information Not complete (some information is lost)\n")
            if string.len(lic_index[1]) ~= 2 then
                ev = avl.pfal(string.format("MSG.Send.USB,0,\"License Type part, Not Complete: %s\"", lic_index[1]))
                data = data..string.format("License Type part, Not Complete: %s\n", lic_index[1])
            end
            if string.len(lic_index[2]) ~= 1 then
                ev = avl.pfal(string.format("MSG.Send.USB,0,\"License Gender part, Not Complete: %s\"", lic_index[2]))
                data = data..string.format("\nLicense Gender part, Not Complete: %s\n", lic_index[2])
            end
            if string.len(lic_index[3]) < 7 and string.len(lic_index[3]) > 8 then
                ev = avl.pfal(string.format("MSG.Send.USB,0,\"License No. part, Not Complete: %s\"", lic_index[3]))
                data = data..string.format("\nLicense No. part, Not Complete: %s\n", lic_index[3])
            end
            if string.len(lic_index[4]) ~= 5 then
                ev = avl.pfal(string.format("MSG.Send.USB,0,\"License Location part, Not Complete: %s\"", lic_index[4]))
